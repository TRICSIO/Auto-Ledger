...
import type { Vehicle, Expense, MaintenanceTask, FuelLog, VehicleDocument } from './data-model';
...
export function deleteExpense(expenseId: string): { success: boolean, vehicleId?: string, message?: string } {
    const expenses = getExpenses();
...
export function setAllData(data: { vehicles: Vehicle[], expenses: Expense[], maintenanceTasks: MaintenanceTask[], fuelLogs: FuelLog[], documents: VehicleDocument[] }) {
    ...
    // Mark data as initialized to prevent overwriting on next load
    window.localStorage.setItem('dataInitialized', 'true');
}
